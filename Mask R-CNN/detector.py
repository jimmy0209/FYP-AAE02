...
Usage:
    ...
    python3 -W ignore detector.py train --model=coco --dataset=../data --class_map=./taco_config/map_1.csv --round 0

    # Continue training a model that you had trained earlier
    python3 -W ignore detector.py train  --dataset=../data --model=last --class_map=./taco_config/map_1.csv --round 0

    # Continue training the last model you trained with image augmentation
    python3 detector.py train --dataset=../data --model=last --round 0 --class_map=./taco_config/map_1.csv --use_aug

    # Test model and visualize predictions image by image
    python3 detector.py test --dataset=../data --model=taco --round 0 --class_map=./taco_config/map_1.csv

    # Run COCO evaluation on a trained model
    python3 detector.py evaluate --dataset=../data --model=taco --round 0 --class_map=./taco_config/map_1.csv
...
TACO_MODEL_PATH = os.path.join(ROOT_DIR, "mask_rcnn_taco_0200.h5")
...
if __name__ == '__main__':
    ...
    if args.command == "train":
        ...
        # Training all layers
        model.train(dataset_train, dataset_val,learning_rate=config.LEARNING_RATE, epochs=200,
                    layers='all', augmentation=augmentation_pipeline)

        # Training last layers100
